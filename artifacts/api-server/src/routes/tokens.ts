...
async function fetchMoralis(): Promise<TrendingToken[]> {
  if (!MORALIS_API_KEY) {
    logger.warn("MORALIS_API_KEY not set, skipping Moralis fetch");
    return [];
  }
  ...
  try {
    ...
    logger.warn({ err }, "Moralis fetch failed");
    return [];
  }
}
...
async function fetchTrendingTokens(): Promise<TrendingToken[]> {
  ...
  if (cachedTokens.length > 0 && now - lastFetchTime < CACHE_TTL) {
    logger.info("Returning cached trending tokens");
    return cachedTokens;
  }

  logger.info("Fetching fresh trending tokens from Moralis and DexScreener");
  ...
  logger.info({ count: cachedTokens.length }, "Trending tokens fetched and cached");
  return cachedTokens;
}
...
router.get("/trending", async (_req, res) => {
  try {
    ...
    res.json({ tokens, count: tokens.length, cached: Date.now() - lastFetchTime < CACHE_TTL });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    logger.error({ err }, "Failed to fetch trending tokens");
    res.status(500).json({ error: message, tokens: cachedTokens || [] });
  }
});
